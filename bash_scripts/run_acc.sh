...
tmux send-keys "rosbag record -a -o acc_$MODE.bag" C-m

# PANE 2: OL demo
tmux select-pane -t 2
tmux send-keys "roscd dbw_genesis_ol_response/scripts" C-m
tmux send-keys "python demo_open_loop_acc.py --mode $MODE" # force user to start this.

# PANE 2: rostopic echo
tmux select-pane -t 3
tmux send-keys "roscd dbw_genesis_ol_response/" C-m
tmux send-keys "rostopic echo /vehicle/imu" # force user to start this.
